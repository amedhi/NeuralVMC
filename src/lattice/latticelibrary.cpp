...
int Lattice::define_lattice(void) 
{
  ...
  if (lname == "SQUARE") {
    ...
    // add sites
    add_basis_site(type=0, coord=vec(0,0,0));         // A-site
    add_basis_site(type=1, coord=vec(0.5*b,0.5*b,0)); // B-site
  ...
  /*------------- 'CHAIN' lattice--------------*/
  ...
  else if (lname == "NICKELATE_4SITE") {
    lid = lattice_id::NICKELATE_4SITE;
    extent[dim3] = Extent{1, boundary_type::open, boundary_type::open, 0.0};
    // basis vectors
    double a = 1.0;
    double b = 2.0;
    set_basis_vectors(a1=vec(b,0,0), a2=vec(0,b,0), a3=vec(0,0,0));
    // sites
    add_basis_site(type=0, coord=vec(0,0,0)); // A (N-layer)
    add_basis_site(type=0, coord=vec(a,a,0)); // A (N-layer)
    add_basis_site(type=1, coord=vec(a,0,0)); // B (N-layer)
    add_basis_site(type=1, coord=vec(0,a,0)); // B (N-layer)

    add_basis_site(type=2, coord=vec(0,0,a)); // A (R-layer)
    add_basis_site(type=2, coord=vec(a,a,a)); // A (R-layer)
    add_basis_site(type=3, coord=vec(a,0,a)); // B (R-layer)
    add_basis_site(type=3, coord=vec(0,a,a)); // B (R-layer)

    // basis vectors
    //A-B bonds in the N-layer
    add_bond(type=0,src=0,tgt=2,offset=pos(0,0,0));
    add_bond(type=0,src=1,tgt=3,offset=pos(1,0,0));
    add_bond(type=0,src=2,tgt=0,offset=pos(1,0,0));
    add_bond(type=0,src=3,tgt=1,offset=pos(0,0,0));
    ...
    //A-B bonds in the R-layer
    add_bond(type=2,src=4,tgt=6,offset=pos(0,0,0));
    add_bond(type=2,src=5,tgt=7,offset=pos(1,0,0));
    add_bond(type=2,src=6,tgt=4,offset=pos(1,0,0));
    add_bond(type=2,src=7,tgt=5,offset=pos(0,0,0));

    add_bond(type=3,src=4,tgt=7,offset=pos(0,0,0));
    add_bond(type=3,src=5,tgt=6,offset=pos(0,1,0));
    add_bond(type=3,src=6,tgt=5,offset=pos(0,0,0));
    add_bond(type=3,src=7,tgt=4,offset=pos(0,1,0));

    //A-A bonds in the N-layer
    add_bond(type=4,src=0,tgt=1,offset=pos(0,0,0));
    add_bond(type=4,src=1,tgt=0,offset=pos(1,1,0));
    add_bond(type=5,src=0,tgt=1,offset=pos(-1,0,0));
    add_bond(type=5,src=1,tgt=0,offset=pos(0,1,0));

    //B-B bonds in the N-layer
    add_bond(type=5,src=2,tgt=3,offset=pos(0,0,0));
    add_bond(type=4,src=2,tgt=3,offset=pos(1,0,0));
    add_bond(type=4,src=3,tgt=2,offset=pos(0,1,0));
    add_bond(type=5,src=3,tgt=2,offset=pos(-1,1,0));

    //B-B bonds in the R-layer
    add_bond(type=7,src=6,tgt=7,offset=pos(0,0,0));
    add_bond(type=6,src=6,tgt=7,offset=pos(1,0,0));
    add_bond(type=6,src=7,tgt=6,offset=pos(0,1,0));
    add_bond(type=7,src=7,tgt=6,offset=pos(-1,1,0));

    //Ni-R bonds
    add_bond(type=8,src=0,tgt=4,offset=pos(0,0,0));
    add_bond(type=8,src=1,tgt=5,offset=pos(0,0,0));
    add_bond(type=8,src=2,tgt=6,offset=pos(0,0,0));
    add_bond(type=8,src=3,tgt=7,offset=pos(0,0,0));
  }
