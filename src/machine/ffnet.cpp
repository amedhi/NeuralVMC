  ...
  const int& input_dim)
{
  if (layers_.size()==0) {
    if (input_dim<1) {
      ...
      // input layer
      layers_.push_back(NeuralLayer(input_dim,"None",input_dim));
      // first layer
      layers_.push_back(NeuralLayer(units,activation,input_dim));
  ...
  if (input_dim==0 || layers_.back().num_units()==input_dim) {
    layers_.push_back(NeuralLayer(units,activation,layers_.back().num_units()));
