...
//---------------------------------------------------------------------------
  ...
  //---------------------------------------------------------------------------
    ...
    if (model.id()==model::model_id::HUBBARD_IONIC || 
      ...
      // different pairing ampl for B-B bonds
      bool have_dSC_BB = false;
      bool have_tpv_BB = false;
      mf_model_.add_parameter(name="tpv_BB",defval=1.0,inputs,info);
      if (info == 0) have_tpv_BB = true;
      ...
      cc.add_type(2,"0");
      cc.add_type(3,"0");
      if (have_tpv_BB == false) {
        cc.add_type(4,"-tpv");
        cc.add_type(5,"-tpv");
      }
      else {
        cc.add_type(4,"-tpv_BB");
        cc.add_type(5,"-tpv_BB");
      }
      ...
      */
      ...
      if (order()==order_t::SC && pair_symm()==pairing_t::DWAVE) {
        ...
        order_name_ = "SC-DXY";
        mf_model_.add_parameter(name="dSC_BB",defval=1.0,inputs,info);
        if (info == 0) have_dSC_BB = true;
        ...
        cc.add_type(2,"-dSC");
        cc.add_type(3,"dSC");
        // B-B bond can have different pairing amplitude
        if (!have_dSC_BB) {
          cc.add_type(4,"-dSC");
          cc.add_type(5,"dSC");
        }
        else {
          cc.add_type(4,"-dSC_BB");
          cc.add_type(5,"dSC_BB");
        }
        mf_model_.add_bondterm(name="pairing", cc, op::pair_create());
      }
      else {
      ...
      if (lattice.id()==lattice::lattice_id::SQUARE_2SITE) {
        add_correlation_bonds(0, {0,0});
        add_correlation_bonds(0, {0,1});
        add_correlation_bonds(4, {2,2});
        add_correlation_bonds(4, {2,3});
        add_correlation_bonds(6, {4,4});
        add_correlation_bonds(6, {4,5});
        set_max_dist(lattice.size1()/2);
      }
      ...
      if (have_dSC_BB) {
        defval = mf_model_.get_parameter_value("dSC_BB");
        varparms_.add("dSC_BB",defval,lb=1.0E-3,ub=2.0,dh=0.01);
      }
      ...
      if (inputs.set_value("tv_variational", true)) {
        ...
        varparms_.add("tpv", defval,lb=0.0,ub=5.0,dh=0.02);
        if (have_tpv_BB == true) {
          defval = mf_model_.get_parameter_value("tpv_BB");
          varparms_.add("tpv_BB", defval,lb=0.0,ub=5.0,dh=0.02);
        }
      }
    }
...
//---------------------------------------------------------------------------
  ...
  else if (lattice.id()==lattice::lattice_id::NICKELATE_4SITE) {
    bool mu_default = inputs.set_value("mu_default", false);
    mf_model_.add_parameter(name="e_R", defval=0.0, inputs);
    mf_model_.add_parameter(name="t", defval=1.0, inputs);
    mf_model_.add_parameter(name="tp", defval=0.0, inputs);
    mf_model_.add_parameter(name="th", defval=0.0, inputs);
    mf_model_.add_parameter(name="dSN", defval=1.0, inputs);
    mf_model_.add_parameter(name="dSR", defval=1.0, inputs);
    mf_model_.add_parameter(name="dAF", defval=1.0, inputs);
    mf_model_.add_parameter(name="mu_N", defval=0.0, inputs, info);
    if (mu_default && info==0) std::cout << " >> alert: conflicting option for `mu`\n";
    mf_model_.add_parameter(name="mu_R", defval=0.0, inputs, info);
    if (mu_default && info==0) std::cout << " >> alert: conflicting option for `mu`\n";

    // Site terms: AF-order & chemical potential in the Ni-layer
    cc.create(4);
    cc.add_type(0, "-(mu_N+dAF)");
    cc.add_type(1, "-(mu_N-dAF)");
    cc.add_type(2, "(e_R-mu_R)");
    cc.add_type(3, "(e_R-mu_R)");
    mf_model_.add_siteterm(name="ni_up", cc, op::ni_up());
    ...
    // bond operators
    cc.create(9);
    cc.add_type(0, "-t");   //A-B bonds in the N-layer (x-direction)
    cc.add_type(1, "-t");   //A-B bonds in the N-layer (y-direction)
    cc.add_type(2, "-t");   //A-B bonds in the R-layer (x-direction)
    cc.add_type(3, "-t");   //A-B bonds in the R-layer (y-direction)
    cc.add_type(4, "-tp");  //A-A and B-B bonds in the N-layer (x+y direction)
    cc.add_type(5, "-tp");  //A-A and B-B bonds in the N-layer (-x+y direction)
    cc.add_type(6, "-tp");  //A-A and B-B bonds in the R-layer (x+y direction)
    cc.add_type(7, "-tp");  //A-A and B-B bonds in the R-layer (-x+y direction)
    cc.add_type(8, "-th");  //N-R bonds
    mf_model_.add_bondterm(name="hopping", cc, op::spin_hop());

    if (order()==order_t::SC && pair_symm()==pairing_t::DWAVE){
      order_name_ = "SC-DWAVE";
      cc.create(9);
      //pairing in the N-layer
      cc.add_type(0, "dSN");
      cc.add_type(1, "-dSN");
      cc.add_type(4, "0");
      cc.add_type(5, "0");
      //pairing in the R-layer
      cc.add_type(2, "dSR");
      cc.add_type(3, "-dSR");
      cc.add_type(6, "0");
      cc.add_type(7, "0");
      ...
      add_correlation_bonds(0, {0,0});
      add_correlation_bonds(0, {0,1});
      add_correlation_bonds(0, {2,2});
      add_correlation_bonds(0, {2,3});
    }
    else {
      throw std::range_error("BCS_State::BCS_State: state undefined for the lattice");
    }
    ...
    // variational parameters
    defval = mf_model_.get_parameter_value("dSN");
    varparms_.add("dSN", defval,lb=1.0E-3,ub=6.0,dh=0.02);
    defval = mf_model_.get_parameter_value("dSR");
    varparms_.add("dSR",defval,lb=0.0,ub=6.0,dh=0.02);
    defval = mf_model_.get_parameter_value("dAF");
    varparms_.add("dAF",defval,lb=0.0,ub=6.0,dh=0.02);
  ...
  Matrix& psi)
{
    ...
    for (int j=0; j<num_kpoints_; ++j) {
      ...
      for (int m=0; m<kblock_dim_; ++m) {
        for (int n=0; n<kblock_dim_; ++n) {
          ...
          // check if amplitudes are not real
#ifdef REAL_WAVEFUNCTION
          //std::cout << "ampl = " << ampl << "\n";
          if (std::abs(ampl.imag())>=1.0E-12) num_cmplx_elems +=1;
#endif
        } 
      }
      q += kblock_dim_;
  ...
  // warn if amplitudes are not real
#ifdef REAL_WAVEFUNCTION
  if (num_cmplx_elems>0) {
    std::cout << " >> warning! BCS_State:: found '"<<num_cmplx_elems<<
