...
void Fermisea::construct_groundstate(void)
{
  ...
  */
  ...
  // Energy gap
  /*
  int next = top_filled_level+1;
  if (next >= ek.size()) next = ek.size()-1;
  double Eg = ek[idx[next]]-ek[idx[top_filled_level]];
  std::cout << "Eg = " << Eg << "\n\n"; 
  std::cout << "states below Fermi\n";
  for (int i=num_particle-10; i<num_particle; ++i) {
    int k, n, s;
    std::tie(k,n,s) = qn_list[idx[i]];
    std::cout << "ek[idx["<<setw(3)<<i<<"]] = "<<setw(10)<<ek[idx[i]]<<setw(4)<<k<<setw(3)<<n<<setw(3)<<s<<"\n";
  }
  std::cout << "states above Fermi\n";
  for (int i=num_particle; i<num_particle+10; ++i) {
    int k, n, s;
    std::tie(k,n,s) = qn_list[idx[i]];
    std::cout << "ek[idx["<<setw(3)<<i<<"]] = "<<setw(10)<<ek[idx[i]]<<setw(4)<<k<<setw(3)<<n<<setw(3)<<s<<"\n";
  }
