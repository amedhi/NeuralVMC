...
public:
  ...
  CorrelationPairs() {}
 ~CorrelationPairs() {}
  void set_min_dist(const int& d) { min_dist_ = d; }
  void set_max_dist(const int& d) { max_dist_ = d; }
  ...
  const int& min_corr_dist(void) const { return min_dist_; }
  const int& max_corr_dist(void) const { return max_dist_; }
...
public:
  ...
  const MF_Model& mf_model(void) const { return mf_model_; }
protected:
  std::string name_;
  int num_sites_{0};
  int num_bonds_{0};
