...
void GW_Projector::get_grad_logp(const vmc::FockBasis& state, RealVector& grad) const
{
  if (default_case_) {
    int nd = 0;
...
double WavefunProjector::dh_ratio(const vmc::FockBasis& state, 
  const int& fr_site, const int& to_site) const
{
...
void WavefunProjector::get_grad_logp(const vmc::FockBasis& state, RealVector& grad) const
{
